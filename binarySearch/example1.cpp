...
int lastOccurence(int arr[],int n, int key){
    ...
    while(s<=e){
        if(arr[mid]==key){
            ans=mid;
            s=mid+1;
            //I have to find my key's occurence on the right hand side, because of which we changed the value of s
        }
        else if(key>arr[mid]) { //go in right of the mid and serach for the key
            s=mid+1;
        }
...
int main(){
    ...
    cout<<"the first occurence is "<<firstOccurence(even,6,3)<<endl;
    cout<<"the last occurence is "<<lastOccurence(even,6,3)<<endl;
return 0;
}
