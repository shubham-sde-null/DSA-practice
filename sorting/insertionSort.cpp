...
for(int i=1;i<n;i++){
    ...
    int current=array[i];
    int j=i-1;
    //we wrote this j ouside of loop because we want to use j to update position, but of we used j ouside it's loop then it does not have any scope and we will get the error.
    for(;j>=0;j--){
        if(array[j]>current){
