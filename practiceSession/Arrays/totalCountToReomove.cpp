...
//             return 1;
//         }
//         if(minIndex<maxIndex && maxIndex<midValue){
...
//         }
//         // cout<<endl;
//         // int totalCount=maxIndex+minIndex;
//         // cout<<"the maxIndex is At:"<<maxIndex<<endl;
//         // cout<<"the minIndex is At:"<<minIndex<<endl;
//         // cout<<"Count to remove minimum and the maximum is :"<<sizeOfVector-counter<<endl;
//         // [-87,60,-30,-67,74,55,76,-53]
//     }
// int main(){
//     vector<int> nums;
//     int count;
...
int minimumDeletions(vector<int> &nums){
    ...
    for(int i=0;i<nums.size();i++){
         ...
         cout<<"size of vector is"<<sizeOfVector<<endl;
    //if both minIndex and max index at left of mid value
    if((minIndex<=midValue && maxIndex<=midValue)){
        if(minIndex<maxIndex){
            for(int i=0;i<=maxIndex;i++){
                    counter++;
            }
    ...
    //if both minIndex and max index at right of mid value
    if((minIndex>=midValue && maxIndex>=midValue)){
        if(minIndex<maxIndex){
            for(int i=sizeOfVector-1;i>=minIndex;i--){
                    counter++;
                    // cout<<"a";
            }
        }
        if(minIndex<maxIndex){
            for(int i=sizeOfVector-1;i>=maxIndex;i--){
                    counter++;
                    // cout<<"b";
            }
        }

        // cout<<"both at right"<<endl;
        // cout<<sizeOfVector<<endl;
    }

    // if min index is at right of mid value and max index is at left of mid value
    if(minIndex>midValue && maxIndex<midValue){
            for(int i=minIndex;i<=sizeOfVector-1;i++){
                a++;
            }
            for(int i=0;i<=maxIndex;i++){
                b++;
            ...
            return result;
            //  cout<<counter<<endl;
        // cout<<"min at left and max at right";
    }
    // if max index is at right of mid value and min index is at left of mid value
    if(minIndex<midValue && maxIndex>midValue){
            for(int i=0;i<=minIndex;i++){
                a++;
            }
            for(int i=maxIndex;i<=sizeOfVector-1;i++){
                b++;
            }
            cout<<"a is:"<<a<<endl;
            cout<<"b is:"<<b<<endl;
            cout<<"this one";
            if(a>b){
                return sizeOfVector-a+1;
            }
            if(b>a){
                return sizeOfVector-b+1;
            }
